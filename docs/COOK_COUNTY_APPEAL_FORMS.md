...
When filing **on behalf of** a property owner, CCAO requires an Attorney/Representative Authorization form. Cook County provides their **own official form**. We use the direct PDF: [attorneyrepresentativeauthorizationform.pdf (VersionId)](https://prodassets.cookcountyassessor.com/s3fs-public/form_documents/attorneyrepresentativeauthorizationform.pdf?VersionId=Lf5aaF.oc6aPygRAoAqn2JS5B56_MAxa). We pre-fill it with user data and they e-sign; or they can download, sign, and upload. Electronic signatures are accepted; notarization is not required. See **`docs/AUTHORIZATION_FORM_RESEARCH.md`** for full research.
