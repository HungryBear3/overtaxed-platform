...
describe("Chicago calendar day", () => {
  ...
  it("treats the last millisecond of a Chicago day as that day", () => {
    ...
    expect(chicagoCalendarDay(Date.parse("2026-06-11T04:59:59.999Z"))).toBe("2026-06-10")
    expect(chicagoCalendarDay(Date.parse("2026-06-11T05:00:00.000Z"))).toBe("2026-06-11")
  })
  ...
  it("handles the spring-forward transition", () => {
    ...
    expect(chicagoCalendarDay(Date.parse("2026-03-08T07:59:00.000Z"))).toBe("2026-03-08")
    expect(chicagoCalendarDay(Date.parse("2026-03-08T08:00:00.000Z"))).toBe("2026-03-08")
    ...
    expect(chicagoCalendarDay(Date.parse("2026-03-09T04:59:00.000Z"))).toBe("2026-03-08")
    expect(chicagoCalendarDay(Date.parse("2026-03-09T05:00:00.000Z"))).toBe("2026-03-09")
  })

  it("handles the fall-back transition", () => {
    // 2026-11-01 02:00 CDT becomes 01:00 CST.
    expect(chicagoCalendarDay(Date.parse("2026-11-01T05:30:00.000Z"))).toBe("2026-11-01")
    expect(chicagoCalendarDay(Date.parse("2026-11-02T05:59:00.000Z"))).toBe("2026-11-01")
    expect(chicagoCalendarDay(Date.parse("2026-11-02T06:00:00.000Z"))).toBe("2026-11-02")
  })
})
...
describe("the approved checkout cutoff", () => {
  ...
  it("refuses a window with exactly two business days left", () => {
    ...
    expect(decision).toMatchObject({ reason: "insufficient_business_days", businessDaysRemaining: 2 })
  })
  ...
  it("refuses when the window closes today", () => {
    ...
    expect(decision.allowed).toBe(false)
    expect(decision).toMatchObject({ reason: "insufficient_business_days", businessDaysRemaining: 0 })
  })
  ...
  it("flips at the exact Chicago midnight, not the UTC one", () => {
    ...
    expect(
      evaluateCheckoutBusinessDayCutoff({ closeDate: "2026-06-11", now: lastInstantOfMonday }).allowed,
    ).toBe(true)
  ...
  it("refuses when an injected holiday removes the third day", () => {
    const base = { closeDate: "2026-06-11", now: at("2026-06-08T14:00:00.000Z") }
  ...
  it("never widens: a longer window is allowed, a shorter one never is", () => {
    ...
    const days = ["2026-06-08", "2026-06-09", "2026-06-10", "2026-06-11", "2026-06-12", "2026-06-15"]
