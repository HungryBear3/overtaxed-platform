...
describe("A. attempt→artifact identity is relationally enforced (same fulfillment)", () => {
  ...
  it("the referenced artifact key is unique", () => {
    expect(model("OTFulfillmentArtifact")).toMatch(
      /@@unique\(\[fulfillmentId,\s*version\]\)/,
    );
  });
...
describe("C. event→attempt binding and unique local sequence", () => {
  ...
  it("the generated SQL emits the event→attempt FK and the unique sequence", () => {
    ...
    expect(SQL).toMatch(
      /CREATE UNIQUE INDEX "ot_delivery_event_fulfillment_id_sequence_key"/,
    );
  });
});
...
describe("migration remains additive and non-destructive", () => {
  it("performs no ALTER/DROP on existing tables and no DML backfill", () => {
    ...
    const alterTargets = [...SQL.matchAll(/ALTER TABLE "([a-z_]+)"/g)].map(
      (m) => m[1],
    );
...
// ---- Remediation-2 G: append-only intra-evidence FKs are non-destructive ----

describe("G. intra-evidence identity FKs never cascade delete/update", () => {
  const fkLine = (name: string) =>
    SQL.split("\n").find((l) => l.includes(name)) ?? "";

  it("attempt→artifact FK uses NO ACTION on both delete and update", () => {
    const line = fkLine(
      "ot_delivery_attempt_fulfillment_id_artifact_version_fkey",
    );
    expect(line).toMatch(/ON DELETE NO ACTION ON UPDATE NO ACTION/);
    expect(line).not.toMatch(/CASCADE/);
    expect(model("OTDeliveryAttempt")).toMatch(
      /artifact\s+OTFulfillmentArtifact\s+@relation\([^)]*onDelete:\s*NoAction[^)]*onUpdate:\s*NoAction/,
    );
  });

  it("event→attempt FK uses NO ACTION on both delete and update", () => {
    const line = fkLine("ot_delivery_event_fulfillment_id_attempt_number_fkey");
    expect(line).toMatch(/ON DELETE NO ACTION ON UPDATE NO ACTION/);
    expect(line).not.toMatch(/CASCADE/);
    expect(model("OTDeliveryEvent")).toMatch(
      /attempt\s+OTDeliveryAttempt\s+@relation\([^)]*onDelete:\s*NoAction[^)]*onUpdate:\s*NoAction/,
    );
  });

  it("parent lifecycle FKs (child→fulfillment, fulfillment→order) still CASCADE", () => {
    for (const name of [
      "ot_fulfillment_order_id_fkey",
      "ot_fulfillment_artifact_fulfillment_id_fkey",
      "ot_delivery_attempt_fulfillment_id_fkey",
      "ot_delivery_event_fulfillment_id_fkey",
    ]) {
      expect(fkLine(name)).toMatch(/ON DELETE CASCADE ON UPDATE CASCADE/);
    }
  });
});
