...
describe("decideDeliverySend — a retry never regenerates (row 7)", () => {
  ...
  // ---- Remediation-2 C: schema-safe (PostgreSQL Int) bounds; no unsafe overflow ----
  it.each([
    9007199254740992,
    ...
    "unsafe/over-range attemptCount %p never authorizes a send",
    (attemptCount) => {
      expect(
        decideDeliverySend({
          status: "ARTIFACT_READY",
  ...
  it("attemptCount at the PostgreSQL Int ceiling cannot authorize an overflowing next value", () => {
    expect(
      decideDeliverySend({
        status: "ARTIFACT_READY",
        attemptCount: 2147483647,
  ...
  it.each([9007199254740992, Number.MAX_SAFE_INTEGER, 2147483648])(
    "unsafe/over-range maxAttempts %p never authorizes a send",
    (maxAttempts) => {
      expect(
        decideDeliverySend({
          status: "ARTIFACT_READY",
...
describe("decideRegeneration — regeneration never sends (row 8)", () => {
  ...
  it.each([
    ...
    "malformed / unsafe / ceiling current version %p fails closed",
  ...
  it("regeneration issues strictly the next version, never an existing/lower one", () => {
    const d = decideRegeneration({
      ...
      hasArtifact: true,
      artifactValid: false,
      currentArtifactVersion: 5,
      explicitRequest: false,
    });
    expect(d).toEqual({
      regenerate: true,
      nextArtifactVersion: 6,
      createsDeliveryAttempt: false,
    });
  });
});
