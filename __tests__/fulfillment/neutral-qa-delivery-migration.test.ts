...
describe("neutral QA/delivery migration authority", () => {
  ...
  it("keeps the applied migration immutable and hardens through forward state", () => {
    const hardening=fs.readFileSync(path.join(process.cwd(),"prisma/migrations/20260916121000_harden_ot_supabase_owner_roles/migration.sql"),"utf8")
    const reconciliation=fs.readFileSync(path.join(process.cwd(),"prisma/migrations/20260916120000_reconcile_ot_neutral_qa_delivery_forward/migration.sql"),"utf8")
    expect(hardening).toContain("ot_neutral_reversal_guard_owner")
    expect(hardening).toContain("owner role % has unsafe attributes")
    expect(hardening).toContain("owner role % has an unexpected membership graph")
    expect(hardening).toContain("REVOKE CREATE ON SCHEMA public")
    expect(reconciliation).toContain("migration 33 reconciliation refused")
    expect(reconciliation).toContain("provider_lookup_attempts")
    expect(reconciliation).toContain("last_provider_lookup_at")
    expect(reconciliation).toContain("last_provider_lookup_result")
    expect(reconciliation).toContain(") <> 21")
    expect(reconciliation).toContain("anon','authenticated','service_role")
    expect(reconciliation).toContain("REVOKE ALL PRIVILEGES ON TABLE public.ot_neutral_customer_zip_attempt")
    expect(reconciliation).toContain("ACL grantors are deliberately omitted")
    expect(reconciliation).toContain("8782889552b478d71c5ab63e2bace721")
    expect(reconciliation).toContain("27b99eb0aada08c4a93990a35f2d0e1c")
    expect(reconciliation).toContain("catalog_hash NOT IN")
    expect(reconciliation).not.toContain("00000000000000000000000000000000")
    expect(reconciliation).not.toContain("_prisma_migrations")
  })

  it("ships native final-state owner-role verification", () => {
    const preflight=fs.readFileSync(path.join(process.cwd(),"scripts/preflight-neutral-report-migration.ts"),"utf8")
    expect(preflight).toContain("ot_commerce_capture_owner")
    expect(preflight).toContain("ot_neutral_reversal_guard_owner")
    expect(preflight).toContain("m.admin_option = true")
    expect(preflight).toContain("m.inherit_option = false")
    expect(preflight).toContain("m.set_option = false")
    expect(preflight).toContain("has_schema_privilege(o.role_name, 'public', 'CREATE')")
    expect(preflight).toContain("Owner-role final-state security invariants are invalid")
    expect(preflight).toContain("Neutral app-reader role invariants are invalid")
    expect(preflight).toContain("Canonical Preview app login invariants are invalid")
    expect(preflight).toContain("canonical_login_edges")
    expect(preflight).toContain("unexpected_edges")
    expect(preflight).toContain("reader_platform_edges")
    expect(preflight).toContain("app_login_platform_edges")
    expect(preflight).toContain("Supabase API roles retain unsafe effective neutral QA/refund privileges")
    expect(preflight).toContain("has_any_column_privilege")
    expect(preflight).toContain("ot_neutral_runtime_order")
    expect(preflight).toContain("ot_packet_download_capability")
    expect(preflight).toContain("ot_neutral_hold_on_settlement_reversal")
  })
