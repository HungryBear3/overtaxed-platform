  ...
  try {
    ...
    try {
      ...
      for (const tableName of [
        ...
        await runtime.query(`savepoint deny_direct_${tableName}`);
        try {
          await runtime.query(`select * from ${tableName} where false`);
        } catch {
          directReadDenied = true;
          await runtime.query(`rollback to savepoint deny_direct_${tableName}`);
        }
        if (!directReadDenied)
          throw new Error(
            `Runtime direct SELECT unexpectedly succeeded for ${tableName}`,
          );
      }
      await runtime.query(
