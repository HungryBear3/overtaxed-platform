...
function readEvidenceReference(
  ...
  if (
    path.basename(reference.file) !== reference.file ||
    !reference.file.toLowerCase().includes(platform) ||
    seenBasenames.has(reference.file) ||
    !/^[0-9a-f]{64}$/.test(reference.sha256)
  )
    throw new Error("Native Vault evidence reference is invalid");
  seenBasenames.add(reference.file);
  const bytes = readProtectedFile(path.join(directory, reference.file));
