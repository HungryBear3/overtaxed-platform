...
import { promoteApprovedNeutralCustomerZip } from "@/lib/fulfillment-runtime/neutral-customer-promotion"
...
export async function POST(request:NextRequest,context:{params:Promise<{orderId:string}>}){
  ...
  try{
    const result=parsed.data.action==="OPEN"?await openNeutralQaReview({orderId,reviewerKey:`admin:${user.id}`}):parsed.data.action==="PROMOTE"?await promoteApprovedNeutralCustomerZip(orderId):await decideNeutralQaReview({orderId,reviewerKey:`admin:${user.id}`,decision:parsed.data.decision,reasonCode:parsed.data.reasonCode,minutesSpent:0})
