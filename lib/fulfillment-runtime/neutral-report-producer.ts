...
export async function produceNeutralReport(input: { orderId: string; propertyPin?: string }) {
  if (process.env.NODE_ENV === "test") {
    return produce({ orderId: input.orderId, propertyPin: input.propertyPin ?? "" })
  }
  ...
  return produce({ orderId: input.orderId, propertyPin: pin }, { active: true, repository: prismaNeutralReportRepository })
}
