...
export async function resolveNeutralOrderAuthority(orderId: string): Promise<NeutralOrderAuthority | null> {
  ...
  const rows = await db.$transaction(async tx=>{
    ...
    admitted AS (UPDATE "ot_neutral_report_reservation" r SET "paid_cohort_position"=COALESCE(r."paid_cohort_position",(SELECT s FROM slot)),"paid_admitted_at"=COALESCE(r."paid_admitted_at",CURRENT_TIMESTAMP),"updated_at"=CURRENT_TIMESTAMP FROM "ot_order" o
    WHERE r."order_id"=o."id" AND o."id"=${orderId} AND (r."paid_cohort_position" IS NOT NULL OR EXISTS(SELECT 1 FROM slot))
      ...
      AND ${trustedPaymentAuthority("o")} AND r."policy_version"=${NEUTRAL_REPORT_COMMERCE_POLICY.version} AND r."status" IN ('RESERVED','STAGED','PROMOTED')
    RETURNING r.*)
    ...
    FROM "ot_order" o JOIN admitted r ON r."order_id"=o."id" LIMIT 1`)
  })
